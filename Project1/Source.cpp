...
void Evolution(char** arr, char** arr2, int rows, int columns) // функция создания жизни и смерти клеток
{
    for (int x = 0; x < rows; ++x) // проходимся по строкам
    {
        for (int y = 0; y < columns; ++y) // проходимся по столбцам
        {
            ...
            arr2[x][y] = arr[x][y]; // копируем состояние клетки из старого мира
            ...
            {
                if (alive_cell == 3) // если живых_соседок == 3
                {
...
int main()
{
    ...
    if (file_to_use.is_open())
    {
        ...
        do
        {
            ...
            std::system("CLS"); // в Windows консоль очищается так
...
/*
 ...
 20 30 программа падает. Точка остановки срабатывает на строчке 215 или 216.
*/